...
def run(args):
    ...
    model.to(args.gpu_num) 

    gt, forecasts = [], []
    for data in tqdm(testloader, total=len(testloader)):
        with torch.no_grad():
            (X, y, categories, colors, fabrics, stores, temporal_features, gtrends), images = data
            X, y, categories, colors, fabrics, stores, temporal_features, gtrends, images = X.to(args.gpu_num), y.to(args.gpu_num), categories.to(args.gpu_num), colors.to(args.gpu_num), fabrics.to(args.gpu_num), stores.to(args.gpu_num), temporal_features.to(args.gpu_num), gtrends.to(args.gpu_num), images.to(args.gpu_num)
